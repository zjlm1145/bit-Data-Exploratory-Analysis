...
for i in tqdm(range(0,16)):
    df = pd.read_parquet("./30G_data_new/part-0000"+str(i)+".parquet")
    lens += len(df)
    # 删除重复值
...
plt.xlabel('Gender')
plt.ylabel('Count')
plt.savefig('./30G_data_new/gender.png')
...
plt.savefig('./30G_data_new/age.png')
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('./30G_data_new/purchase.png')
...
plt.tight_layout()
plt.savefig('./30G_data_new/income.png')
